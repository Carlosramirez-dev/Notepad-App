  ...
  <v-container>
    <v-row>
      <v-col cols="12" sm="6" md="6" v-if="formAgregar">
        <v-card class="mx-auto mb-3 pa-3" max-width="500">
      ...
      <v-col cols="12" sm="6" md="6">
        <v-card
          class="mx-auto mb-3"
          max-width="500"
          v-for="(item, index) in listPlaces"
          :key="index"
        >
          <v-img lass="white--text align-end" height="200px" v-bind:src="item.image"></v-img>
          <v-card-title>{{ item.titulo }}</v-card-title>
          <v-card-text class="text--primary">
            <div>{{ item.descripcion }}</div>
          </v-card-text>
          <v-card-actions>
            <v-btn color="warning" text @click="edit(index)">
              <v-icon left dark>mdi-pencil</v-icon>Edit
            </v-btn>
            <v-btn color="error" text @click="deletePlace(index)">
              <v-icon left dark>mdi-delete</v-icon>Delete
            </v-btn>
          </v-card-actions>
        </v-card>
      </v-col>
    ...
    <!-- Snackbar -->
    <v-snackbar v-model="snackbar" :color="pintar" type="error">
